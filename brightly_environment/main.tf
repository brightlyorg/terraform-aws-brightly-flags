...
locals {
  name_prefix = "brightly-${var.project_name}-${var.env.name}"
}

# aws resources
resource "aws_secretsmanager_secret" "brightly_sdk_key_secret" {
  name = "${local.name_prefix}-sdk-key"
  tags = var.aws_tags
}

resource "random_password" "brightly_sdk_key_password" {
  length           = 16
  min_special      = 16
...
resource "aws_secretsmanager_secret_version" "brightly_sdk_key_secret_version" {
  secret_id     = aws_secretsmanager_secret.brightly_sdk_key_secret.id
  secret_string = "sdk-${var.env.name}-${random_password.brightly_sdk_key_password.result}"
}

resource "aws_secretsmanager_secret" "brightly_mobile_key_secret" {
  name = "${local.name_prefix}-mob-key"
  tags = var.aws_tags
}

resource "random_password" "brightly_mobile_key_password" {
  length           = 16
  min_special      = 16
  override_special = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789"
  special          = true
...
resource "aws_secretsmanager_secret_version" "brightly_mobile_key_secret_version" {
  secret_id     = aws_secretsmanager_secret.brightly_mobile_key_secret.id
  secret_string = "mob-${var.env.name}-${random_password.brightly_mobile_key_password.result}"
}

# github resources
resource "github_repository_file" "brightly_flags_project" {
...
resource "github_repository_file" "brightly_flags_readme" {
  ...
  content             = <<EOF
# Brightly Flags for `${var.project_name}` project, `${var.env.name}` environment
...
```
${aws_secretsmanager_secret_version.brightly_sdk_key_secret_version.secret_string}
```
...
- <details>
  ...
  ```golang
      ...
      ldClient, err := ld.MakeCustomClient("${aws_secretsmanager_secret_version.brightly_sdk_key_secret_version.secret_string}", brightlyConfig, 10*time.Second)
  ```
  </details>
...
### SDK Key
* SDK Key value: `${aws_secretsmanager_secret_version.brightly_sdk_key_secret_version.secret_string}`
* AWS secret arn: `${aws_secretsmanager_secret_version.brightly_sdk_key_secret_version.arn}`
* AWS secret name: `${aws_secretsmanager_secret.brightly_sdk_key_secret.name}`
* AWS: Get secret via cli: `aws secretsmanager get-secret-value --secret-id ${aws_secretsmanager_secret.brightly_sdk_key_secret.name}  | jq -r .SecretString`

### Mobile Key
* Mobile Key value: `${aws_secretsmanager_secret_version.brightly_mobile_key_secret_version.secret_string}`
* Aws secret arn: `${aws_secretsmanager_secret_version.brightly_mobile_key_secret_version.arn}`
* Aws secret name: `${aws_secretsmanager_secret.brightly_mobile_key_secret.name}`
* Get secret using aws cli: `aws secretsmanager get-secret-value --secret-id ${aws_secretsmanager_secret.brightly_mobile_key_secret.name}  | jq -r .SecretString`
